...
function BriefContent() {
  ...
  // Fetch brief_links record from Supabase
  useEffect(() => {
    if (!token) return
    ...
    supabase
      ...
      .single()
      .then(({ data }) => {
        if (data) setBriefLink(data)
      })
  }, [token])

  // Fetch Hub metadata if token is in query params (backward compat)
  const fetchHubMetadata = useCallback(async (hubToken: string) => {
    try {
      const res = await fetch(`${DOCS_HUB_URL}/api/links/${hubToken}`)
      if (res.ok) {
  ...
  useEffect(() => {
    ...
    if (hubToken) {
      fetchHubMetadata(hubToken)
    }
  }, [searchParams, fetchHubMetadata])

  // Load saved form data
  useEffect(() => {
    const savedData = loadFormData(token)
    if (savedData) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      // Include brief_links creator info
      if (briefLink) {
        payload._creator_email = briefLink.creator_email
        payload._creator_name = briefLink.creator_name
        payload._client_email = briefLink.client_email
        payload._brief_token = briefLink.token
      }

      // Include Hub metadata if available (backward compat)
      if (hubMetadata) {
      ...
      // Mark Hub link as completed (backward compat)
